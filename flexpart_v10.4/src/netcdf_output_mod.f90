...
module netcdf_output_mod
  ...
  public :: writeheader_netcdf,concoutput_surf_nest_netcdf,concoutput_netcdf,&
       &concoutput_nest_netcdf,concoutput_surf_netcdf,nf90_err
...
subroutine concoutput_netcdf(itime,outnum,gridtotalunc,wetgridtotalunc,drygridtotalunc)
  ...
  integer             :: ncid,kp,ks,kz,ix,jy,iix,jjy,kzz,kzzm1,ngrid, nh
  integer             :: nage,i,l,jj
  ...
  if (tpointer.eq.1) then
    ...
    ! do nh=1,nhour_10d
    !   call nf90_err(nf90_put_var( ncid, sfromatimeID, -3600*nh, (/ nh /)))
    ! end do
  endif
      ...
      do nage=1,nageclass
        ...
        do jy=0,numygridn-1
          do ix=0,numxgridn-1
            ...
            ! ix_mdomain = int((outlon0n + ix*dxoutn + 180 - 1)/dxoutn)
            ! jy_mdomain = int((outlat0n + jy*dyoutn + 90)/dyoutn)
            ! hmix_ndomain = data_in_onehour(ix_mdomain,jy_mdomain)    
            ! hmix_2dn(ix,jy) = hmix_ndomain
            

            ! write (*,*) 'outlon0n  : ',outlon0n
            ! write (*,*) 'outlat0n  : ', outlat0n
            ! write (*,*) 'ix  : ', ix
            ! write (*,*) 'jy  : ',jy
            ! write (*,*) 'dxoutn  : ',dxoutn
            ! write (*,*) 'dyoutn  : ',dyoutn
            ! write (*,*) 'dyout  : ',dyout
            ! write (*,*) 'dxout  : ',dxout
            ! write (*,*) 'outlon0  : ',outlon0
            ! write (*,*) 'outlat0  : ', outlat0
            ! write (*,*) 'numygridn  : ',numygridn
            ! write (*,*) 'numxgridn  : ',numxgridn

            ! write (*,*) 'x  : ',(outlon0n + ix*dxoutn + 180 - 1)/dxout
            ! write (*,*) 'y  : ',(outlat0n + jy*dyoutn + 90)/dyout
            ! write (*,*) 'int(x)  : ', int((outlon0n + ix*dxoutn + 180 - 1)/dxout)
            ! write (*,*) 'int(y)  : ', int((outlat0n + jy*dyoutn + 90)/dyout)
            ! write (*,*) 'hmix_ndomain  : ',hmix_ndomain
            ! -------------------- end --------------------
